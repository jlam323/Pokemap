...
export const drawPixelSprite = (
  ...
  if (images) {
    ...
    if (spriteName) {
      scaleBaseValue = NPC_SPRITE_CONFIGS[spriteName]?.scale || (spriteImages[spriteName] ? 1 : DEFAULT_NPC_SCALE);
    }
...
const drawSheetSprite = (
  ...
  const columnsInSheet = Math.floor((sheetImg.width - padding + spacing) / (blockWidth + spacing));
  ...
  const scale = NPC_SPRITE_CONFIGS[spriteName || '']?.scale || DEFAULT_NPC_SCALE;
